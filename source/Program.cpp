...
std::tuple<GLenum, std::vector<char>> GLmm::Program::GetBinary() const
{
    if (!ogl_ext_ARB_get_program_binary)
        throw std::runtime_error("GL_ARB_get_program_binary is not supported");
...
std::vector<GLenum> GLmm::GetProgramBinaryFormats()
{
    ...
    static_assert(sizeof(GLint) == sizeof(GLenum), "Cannot get program binaries as GLenum");
