...
public:
    ...
    */
    void SetData(GLenum Target, std::size_t Size, const GLubyte* Data, GLenum Usage);
    ...
    /** Transfer the data to the buffer object.
        ...
        \param Usage Which policy to use for storing.
    */
    template <class T> void SetData(GLenum Target, const std::vector<T>& Data, GLenum Usage)
    {
#if _MSC_VER >= 1600
        static_assert(std::is_trivially_copyable<T>::value, "T needs to have a trivial copy-constructor");
#endif
        SetData(Target, sizeof(T) * Data.size(), reinterpret_cast<const GLubyte*>(Data.data()), Usage);
    }
    ...
    /** Transfer the data to the buffer object.
        ...
        \param Usage Which policy to use for storing.
    */
    template <class T, std::size_t N> void SetData(GLenum Target, const std::array<T, N>& Data, GLenum Usage)
    {
#if _MSC_VER >= 1600
        static_assert(std::is_trivially_copyable<T>::value, "T needs to have a trivial copy-constructor");
#endif
        SetData(Target, sizeof(T) * N, reinterpret_cast<const GLubyte*>(Data.data()), Usage);
    }

    /** Transfer a part of the data to the buffer object.
        Does not reallocate.
    */
    void SetSubData(GLenum Target, std::size_t Size, std::size_t Offset, const GLubyte* Data);
